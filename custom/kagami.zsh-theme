...
if [ "$ZSH_THEME_NO_SPECIAL_CHARACTERS" != "true" ]
then
  ZSH_THEME_GIT_PROMPT_CLEAN=" $KAGAMI_GIT_CLEAN_COLOR✓"
  ZSH_THEME_GIT_PROMPT_DIRTY=" $KAGAMI_GIT_DIRTY_COLOR✗"
else
  ZSH_THEME_GIT_PROMPT_CLEAN=" $KAGAMI_GIT_CLEAN_COLOR""o"
  ZSH_THEME_GIT_PROMPT_DIRTY=" $KAGAMI_GIT_DIRTY_COLOR""x"
fi
...
if which rvm-prompt &> /dev/null; then
  KAGAMI_RVM_="$KAGAMI_BRACKET_COLOR‹%{$reset_color%}$KAGAMI_RVM_COLOR\${\$(~/.rvm/bin/rvm-prompt i v g)#ruby-}$KAGAMI_BRACKET_COLOR›%{$reset_color%} "
# else
#   if which rbenv &> /dev/null; then
#     KAGAMI_RVM_="$KAGAMI_BRACKET_COLOR"["$KAGAMI_RVM_COLOR\${\$(rbenv version | sed -e 's/ (set.*$//' -e 's/^ruby-//')}$KAGAMI_BRACKET_COLOR"]"%{$reset_color%}"
#   fi
fi
...
if [ "$ZSH_THEME_NO_SPECIAL_CHARACTERS" != "true" ]
then
  KAGAMI_PROMPT="$KAGAMI_BRACKET_COLOR➭ "
else
  KAGAMI_PROMPT="$KAGAMI_BRACKET_COLOR> "
fi
